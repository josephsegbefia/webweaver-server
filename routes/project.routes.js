...
router.post(
  '/portfolios/:uniqueIdentifier/projects',
  isAuthenticated,
  async (req, res, next) => {
    try {
      const {
      ...
      res
        .status(200)
        .json({ newProject, message: 'Nice work on adding a new project!' });
    } catch (error) {
      console.log(error);
    ...
    .then((portfolio) => {
      ...
      Portfolio.findOne({ uniqueIdentifier })
        ...
        .then((paginatedPortfolio) => {
          res
            .status(200)
            .json({ projects: paginatedPortfolio.projects, totalPages });
        })
        .catch((error) =>
          res.status(500).json({ message: 'Oops, something went wrong!' })
        );
...
router.get(
  '/portfolios/:uniqueIdentifier/projects/:projectId',
  (req, res, next) => {
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      const projectToUpdate = portfolio.projects.find(
      ...
      if (!projectToUpdate) {
        res
          .status(404)
          .json({ message: 'Project was not found in your portfolio' });
      }
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      const portfolio = await Portfolio.findOne({ uniqueIdentifier }).populate(
        'projects'
      );
      ...
      if (projectIndex === -1) {
        return res
          .status(404)
          .json({ message: 'Project not found in your portfolio' });
      }
